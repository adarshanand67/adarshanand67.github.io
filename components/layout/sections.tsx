                    ...
                    <div className="w-full max-w-4xl mx-auto flex flex-col gap-4 glass p-6 sm:p-8 rounded-2xl animate-fade-in relative">
                        ...
                        <div className="flex flex-col md:flex-row items-center gap-6 md:gap-8">
                            ...
                            <div className="text-center md:text-left flex-1">
                                <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold bg-gradient-to-r from-green-600 to-emerald-600 dark:from-green-400 dark:to-emerald-400 bg-clip-text text-transparent mb-4 tracking-tight">
                                    {profile.name}
                                </h1>
                                <SystemStatus />
                            </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 pt-6">
                            <Link href={`https://${siteConfig.contact.linkedin}`} target="_blank" className="group relative overflow-hidden glass hover:bg-white/50 dark:hover:bg-white/10 rounded-xl p-4 transition-all duration-300 hover:shadow-lg hover:shadow-green-500/10 hover:-translate-y-1">
                                <div className="flex items-center gap-4">
                                    <div className="shrink-0 w-10 h-10 bg-green-500 rounded-lg flex items-center justify-center group-hover:rotate-6 transition-transform">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="text-[10px] text-green-600 dark:text-green-400 font-bold uppercase tracking-wider mb-1">LinkedIn</div>
                                        <div className="text-sm text-gray-900 dark:text-white font-semibold truncate">{siteConfig.contact.linkedin.split('/').pop()}</div>
                                    </div>
                                </div>
                            </Link>

                            <div onClick={() => { navigator.clipboard.writeText(siteConfig.contact.email); setCopied(true); setTimeout(() => setCopied(false), 2000); }} className="group relative overflow-hidden glass hover:bg-white/50 dark:hover:bg-white/10 rounded-xl p-4 transition-all duration-300 hover:shadow-lg hover:shadow-green-500/10 hover:-translate-y-1 cursor-pointer">
                                <div className="flex items-center gap-4">
                                    <div className="shrink-0 w-10 h-10 bg-green-500 rounded-lg flex items-center justify-center group-hover:rotate-6 transition-transform">
                                        {copied ? <Check className="w-5 h-5 text-white" /> : <Copy className="w-5 h-5 text-white" />}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <div className="text-[10px] text-green-600 dark:text-green-400 font-bold uppercase tracking-wider mb-1">{copied ? "Copied!" : "Email"}</div>
                                        <div className="text-sm text-gray-900 dark:text-white font-semibold truncate">{siteConfig.contact.email}</div>
                                    </div>
                                </div>
                            </div>

                            <Link href={`https://${siteConfig.contact.github}`} target="_blank" className="group relative overflow-hidden glass hover:bg-white/50 dark:hover:bg-white/10 rounded-xl p-4 transition-all duration-300 hover:shadow-lg hover:shadow-green-500/10 hover:-translate-y-1">
                                <div className="flex items-center gap-4">
                                    <div className="shrink-0 w-10 h-10 bg-green-500 rounded-lg flex items-center justify-center group-hover:rotate-6 transition-transform">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="text-[10px] text-green-600 dark:text-green-400 font-bold uppercase tracking-wider mb-1">GitHub</div>
                                        <div className="text-sm text-gray-900 dark:text-white font-semibold truncate">{siteConfig.contact.github.split('/').pop()}</div>
                                    </div>
                                </div>
                            </Link>
                        </div>
...
export function Experience({ items }: { items: any[] }) {
    ...
    return (
        <div className="mb-4 font-mono max-w-6xl mx-auto px-4 md:px-6" id="experience">
            <SectionHeader
                title="Experience"
...
export function ShelvesSection() {
    ...
    return (
        <div className="section max-w-6xl mx-auto px-4 md:px-6 mb-8">
                    ...
                    <section className="font-mono">
                        <div className="w-full text-left group mb-3">
                            ...
                            <div className="flex items-center gap-2 text-gray-600 dark:text-gray-400 text-sm">
                                <span className="text-green-500 font-bold">$</span><span>tree -d -L 1 ~</span>
                        ...
                        <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isExpanded ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'}`}>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
                                {shelves.map((shelf) => (
                                    <Link key={shelf.name} href={shelf.path} className="group/item flex items-start gap-3 p-3 rounded-lg border border-gray-200/50 dark:border-gray-800/50 hover:bg-gray-100/50 dark:hover:bg-gray-800/50 hover:border-green-500/30 transition-all duration-300">
                                        <div className={`p-2 rounded-lg bg-gray-100 dark:bg-gray-800 group-hover/item:scale-110 transition-transform duration-300 ${shelf.color}`}><shelf.icon size={20} /></div>
                                        <div>
                                            <div className="font-bold text-gray-700 dark:text-gray-200 group-hover/item:text-green-600 dark:group-hover/item:text-green-400 transition-colors">{shelf.name}/</div>
                                            <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">{shelf.description}</div>
                                        </div>
                                    </Link>
                                ))}
                            </div>
