...
import { SectionHeader } from "./layoutUI";
...
function useTerminal() {
    const { setLines, isIntroDone, setIsIntroDone, isExpanded, position, setPosition, isDragging, setIsDragging } = useStore();
    ...
    return { handleDragStart, inputRef: useRef<HTMLInputElement>(null), containerRef: useRef<HTMLDivElement>(null) };
}
...
function useCommandExecutor() {
    ...
    const { setLines, setHistory, setHistoryIndex, passwordMode, setPasswordMode, setInput, history, todos, addTodo, toggleTodo, removeTodo, clearTodos } = useStore();
    ...
    const executeCommand = useCallback(async (cmd: string) => {
        if (passwordMode) {
            ...
            setTimeout(() => {
                setLines((prev: string[]) => [...prev, success ? "Access Granted. Welcome, Administrator." : "Access Denied."]);
                if (success) setLines((prev: string[]) => [...prev, "Developer Mode: Enabled"]);
            }, 800);
            return;
        }

        if (cmd.trim()) { setHistory((prev) => [cmd, ...prev]); setHistoryIndex(-1); }
        ...
        const context = { setPasswordMode, router, setTheme, setInput, history, todos, addTodo, toggleTodo, removeTodo, clearTodos, setLines: setLines as any };
        ...
        try {
            for (const part of pipeParts) {
                ...
                if (!commandName || !commands[commandName]) {
                    if (commandName) setLines((prev: string[]) => [...prev, `Command not found: ${commandName}`]);
                    continue;
                }
                await commands[commandName].execute(args, context, currentInput);
            }
        } catch (error) { setLines((prev: string[]) => [...prev, `Error executing command.`]); }
    }, [passwordMode, setPasswordMode, setLines, setHistory, setHistoryIndex, router, setTheme, setInput, history, todos, addTodo, toggleTodo, removeTodo, clearTodos]);
...
function useKeyboard(executeCommand: (cmd: string) => void) {
    ...
    return {
        handleKeyDown: useCallback((e: React.KeyboardEvent<HTMLInputElement>) => {
            if (e.key === "Enter") { executeCommand(input); setInput(""); }
                ...
                let candidates = parts.length === 1 ? Object.keys(commands) : ["cd", "open"].includes(cmd) ? [...directories] : [...Object.keys(mockFiles), ...directories];
                const matches = candidates.filter(c => c.toLowerCase().startsWith(currentToken.toLowerCase()));
                if (matches.length === 1) { parts[parts.length - 1] = matches[0]!; setInput(parts.join(" ") + (parts.length === 1 ? " " : "")); }
            }
...
function TerminalHeader({ onMouseDown }: { onMouseDown: (e: React.MouseEvent) => void }) {
    return (
        <div onMouseDown={onMouseDown} className="bg-white/50 dark:bg-white/5 px-4 h-8 flex items-center gap-2 border-b border-white/20 dark:border-white/10 cursor-grab active:cursor-grabbing select-none">
...
function TerminalContent({ lines, isIntroDone, passwordMode, input, containerRef, inputRef, handleKeyDown, onChange, onBlur }: any) {
    return (
        <div ref={containerRef} className="p-4 text-gray-800 dark:text-gray-300 h-[400px] overflow-y-auto overflow-x-auto scrollbar-thin scrollbar-thumb-gray-400 dark:scrollbar-thumb-gray-700 scrollbar-track-transparent">
            {lines.map((line: string, i: number) => (
                <div key={i} className={`whitespace-pre leading-snug tracking-wide ${line.startsWith("$ ") ? "text-foreground font-bold" : ""}`}>
            ...
            {isIntroDone && (
                <div className="flex items-center">
                    ...
                    />
                    {passwordMode && input.length === 0 && <span className="animate-pulse text-foreground">▊</span>}
                </div>
            )}
        </div>
    );
...
export function Terminal() {
    const { lines, isIntroDone, input, setInput, passwordMode, isExpanded, setIsExpanded, position, isDragging } = useStore();
    ...
    return (
        <div className="w-full max-w-7xl relative" onClick={() => isExpanded && !isDragging && isIntroDone && inputRef.current?.focus()}>
                ...
                <section className="font-mono">
                    ...
                    <div className={`transition-all duration-500 ease-in-out ${isExpanded ? "opacity-100 max-h-[1000px]" : "opacity-0 max-h-0 overflow-hidden"}`}>
                        ...
                        >
                            ...
                            <TerminalContent lines={lines} isIntroDone={isIntroDone} passwordMode={passwordMode} input={input} containerRef={containerRef} inputRef={inputRef} handleKeyDown={handleKeyDown} onChange={(e: any) => setInput(e.target.value)} onBlur={(e: any) => setInput(e.target.value)} />
                        </div>
                    </div>
                </section>
            </div>
...
export function TerminalPreloader() {
    ...
    useEffect(() => { if (!isIntroDone) { setLines(introLines()); setIsIntroDone(true); } }, [isIntroDone, setLines, setIsIntroDone]);
    return null;
}
