...
export function UniversalShelf({ config, items }: UniversalShelfProps) {
    ...
    const { filteredItems, randomizerItems, searchQuery, setSearchQuery, animeSelectedTag, setAnimeSelectedTag } = useShelfFilter(items, config.type, strategy);
    ...
    return (
        <div className="section max-w-6xl mx-auto px-6 md:px-12 mt-12 mb-24 font-mono relative">
            ...
            <ShelfHeader title={config.title} description={config.description} count={filteredItems.length} searchValue={searchQuery} onSearchChange={setSearchQuery} searchPlaceholder={config.searchPlaceholder} items={randomizerItems} onPickRandom={handlePickRandom} />

            {config.type === ShelfType.Anime && (
                <AnimeTagFilter items={items} selectedTag={animeSelectedTag} onTagSelect={setAnimeSelectedTag} showClear={!!(searchQuery || animeSelectedTag)} onClear={() => { setSearchQuery(""); setAnimeSelectedTag(null); }} />
            )}

            {filteredItems.length === 0 ? (
                <div className="py-24 text-center text-gray-500">No items found matching "{searchQuery}"</div>
            ) : (
            ...
            <AnimatePresence>
                {store.hobbySelectedItem && <HobbyModal item={store.hobbySelectedItem} onClose={() => store.setHobbySelectedItem(null)} />}
                {store.bookSelectedItem && <BookModal item={store.bookSelectedItem} onClose={() => store.setBookSelectedItem(null)} />}
                {store.animeSelectedItem && <AnimeModal item={store.animeSelectedItem} onClose={() => store.setAnimeSelectedItem(null)} onTagClick={(tag) => { setAnimeSelectedTag(tag); store.setAnimeSelectedItem(null); }} />}
            </AnimatePresence>
        </div>
    );
}
