...
export function XRayWrapper({
    ...
    const handleMouseMove = (e: React.MouseEvent) => {
        ...
        // Calculate relative position for the tooltip to avoid clipping
            ...
            <AnimatePresence>
                {isHovered && (
                    <div
                        ...
                        style={{
                            left: mousePos.x + 320 > (typeof window !== 'undefined' ? window.innerWidth : 1000)
                                ? mousePos.x - 310 // Show on left if near right edge
                                : mousePos.x + 15, // Show on right normally
                            top: mousePos.y + 300 > (typeof window !== 'undefined' ? window.innerHeight : 1000)
                                ? mousePos.y - 290 // Show above if near bottom edge
                                : mousePos.y + 15  // Show below normally
                        }}
                    >
                        <motion.div
                            ...
                            className="w-[300px] bg-black/90 border border-green-500/50 rounded-lg shadow-2xl backdrop-blur-md overflow-hidden text-left"
                        >
                            {/* Header */}
