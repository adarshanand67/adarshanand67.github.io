/**
 * @fileoverview Consolidated API module - data fetching for all content types.
 */
...
export const parseFrontmatter = (content: string) => {
    try {
        if (!content || typeof content !== "string") {
            logError(new Error("Invalid content provided to parseFrontmatter"), { contentType: typeof content });
            return { data: {}, content: "" };
        }
        ...
        if (frontmatterContent) {
            frontmatterContent.split("\n").forEach((line) => {
                try {
                    ...
                    if (k && k.trim()) {
                        data[k.trim()] = v.join(": ").trim().replace(/^['"](.*)['"']$/, "$1");
                    }
                } catch (error) {
    ...
    try {
        ...
        const blogs = await Promise.all(
            markdownFiles.map(async (f) => {
                try {
                    const [fileContent, fileError] = await safeAsync(fs.readFile(path.join(dir, f), "utf8"));
        ...
        return blogs.filter((blog) => blog !== null).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    } catch (error) {
...
export async function getPost(slug: string) {
    try {
        if (!slug || typeof slug !== "string") throw new AppError("Invalid slug provided", "INVALID_SLUG");

        const sanitizedSlug = slug.replace(/[^a-zA-Z0-9-_]/g, "");
        if (sanitizedSlug !== slug) return null;

        const [fileContent, fileError] = await safeAsync(fs.readFile(path.join(process.cwd(), "data", "blogs", `${sanitizedSlug}.md`), "utf8"));
        if (fileError || !fileContent) return null;
...
export const getProfile = async () => {
    if (!profileData) throw new AppError("Profile data not found", "PROFILE_NOT_FOUND");
    return profileData;
};

export const getExperiences = async () => {
    if (!experiencesData || !Array.isArray(experiencesData)) throw new AppError("Experience data not found", "EXPERIENCE_NOT_FOUND");
    return experiencesData;
};

export const getPapers = async () => {
    if (!papersData || !Array.isArray(papersData)) throw new AppError("Papers data not found", "PAPERS_NOT_FOUND");
    return papersData;
};

export const getBooks = async () => {
    if (!booksData || !Array.isArray(booksData)) throw new AppError("Books data not found", "BOOKS_NOT_FOUND");
    return booksData;
};
...
export const getHobby = async () => {
    if (!hobbyData || !Array.isArray(hobbyData)) throw new AppError("Hobby data not found", "HOBBY_NOT_FOUND");
    return hobbyData;
};

export const getAnime = async (): Promise<AnimeItem[]> => {
    if (!anime || !Array.isArray(anime)) throw new AppError("Anime data not found", "ANIME_NOT_FOUND");

    return (anime as AnimeItem[]).map((item, index) => {
        if (!item || typeof item !== "object") throw new AppError(`Invalid anime item at index ${index}`, "INVALID_ANIME_ITEM");
        return {
            ...item,
