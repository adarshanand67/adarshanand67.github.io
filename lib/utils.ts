/**
 * @fileoverview Consolidated utility functions.
 */
...
const bookGradients = [
    "from-red-900 to-red-950", "from-blue-900 to-blue-950", "from-green-900 to-green-950", "from-amber-900 to-amber-950",
    "from-slate-800 to-slate-900", "from-purple-900 to-purple-950", "from-indigo-900 to-indigo-950", "from-rose-900 to-rose-950",
];
...
export async function retryWithBackoff<T>(fn: () => Promise<T>, maxRetries = 3, delayMs = 1000): Promise<T> {
    let lastError: Error | null = null;
    for (let i = 0; i < maxRetries; i++) {
        const [result, error] = await safeAsync(fn());
        if (!error) return result as T;
        lastError = error;
        if (i < maxRetries - 1) await new Promise((resolve) => setTimeout(resolve, delayMs * Math.pow(2, i)));
    }
